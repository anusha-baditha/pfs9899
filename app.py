...
def create():
    if request.method=='POST':
        print(request.form)
        username=request.form['name'] #anusha
        ...
        if result[0]==0:
            gotp=genotp()
            udata={'username':username,'usermail':uemail,'pword':password,'otp':gotp}
    ...
    if not session.get('user'):
        if request.method=='POST':
            ...
            if  bdata[0]==1:
                ...
                bpassword=cursor.fetchone()  #(0x31313100000000000000,)
...
def viewnotes(nid):
    if session.get('user'):
        try:
            ...
            # (1,'python','asdfghjklertyui','2024-12-16 11:14:25',2)
        except Exception as e:
            print(e)
            flash('No data found')
            return redirect(url_for('dashboard'))
...
def viewallfiles():
    if session.get('user'):
        try:
            ...
            filesdata=cursor.fetchall()  #[(1,'python','2024-12-16 11:14:25',),(2,'python  ','2024-12-16 11:14:53',)]
        except Exception as e:
            print(e)
            flash('No data found')
            return redirect(url_for('dashboard'))    
...
def getexceldata():
    if session.get('user'):
        try:
            ...
            columns=['Notesid','Title','Content','Creted_time']
            array_data.insert(0,columns)
            #[['Notesid','Title','Content','Creted_time'],[1, 'python', '2024-12-16 11:14:25'], [2, 'python  ', '2024-12-16 11:14:53']]
